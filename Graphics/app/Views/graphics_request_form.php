...
<head>
  ...
  <title>Graphics Request Form</title>
</head>
...
<h2><em>Graphics Request Form</em></h2>
<h3><em>(If you are having trouble submitting, try using Google Chrome.)</em></h3>
<hr>
<hr><br>

<?php
echo form_open_multipart('form_graph');

if ($validation !== '') {
  if ($validation->hasError('first')) : echo "<div class=two>" . $validation->getError('first') . "</div>";
endif;
}
echo form_label('First: ', 'first') . "<br>" . form_input($first) . "<br><br>";

if ($validation !== '') {
  if (($validation->hasError('last'))) : echo "<div class=two>" . $validation->getError('last') . "</div>";
  endif;
}
echo form_label('Last: ', 'last') . "<br>" . form_input($last) . "<br><br>";

if ($validation !== '') {
  if (($validation->hasError('email'))) : echo "<div class=two>" . $validation->getError('email') . "</div>";
endif;
}
echo form_label('Email: ', 'email') . "<br>" . form_input($email) . "<br><br>";

if ($validation !== '') {
  if (($validation->hasError('time'))) : echo "<div class=two>" . $validation->getError('time') . "</div>";
endif;
}

if ($validation !== '') {
  if (($validation->hasError('phone'))) : echo "<div class=two>" . $validation->getError('phone') . "</div>";
endif;
}
echo form_label('Phone: ', 'phone') . "<br>" . form_input($phone) . "<br><br>";

if ($validation !== '') {
  if (($validation->hasError('title'))) : echo "<div class=two>" . $validation->getError('title') . "</div>";
endif;
}
echo form_label('Title: ', 'title') . "<br>" . form_input($title) . "<br><br>";

if ($validation !== '') {
  if (($validation->hasError('eventDate'))) : echo "<div class=two>" . $validation->getError('eventDate') . "</div>";
  endif;
}
echo form_label('Event Date: ', 'eventDate') . "<br>" . form_input($eventDate) . "<br><br>";

echo form_label('Event Start Time: ', 'startTime') . "<br>";
echo form_dropdown('startHour', $startHour, 1) . ": " .
     form_dropdown('startMinute', $startMinute, 00) . 
     form_dropdown('startAMPM', $startAMPM, 'AM') . "<br><br>";

echo form_label('Event End Time: ', 'endTime') . "<br>";
echo form_dropdown('endHour', $endHour, 1) . ": " .
     form_dropdown('endMinute', $endMinute, 00) . 
     form_dropdown('endAMPM', $endAMPM, 'PM') . "<br><br>";

echo '<b><u>Full Day Event?</b></u><br>' .

     form_radio($allDayYes) . form_label('Yes', 'yes') . "<br>" .
     form_radio($allDayNo) . form_label('No', 'no') . "<br><br>"; 

  if ($validation !== '') {
    if (($validation->hasError('building'))) : echo "<div class=two>" . $validation->getError('building') . "</div>";
    endif;
  }
  echo form_label('Building and floor: ', 'building') . "<br>" . form_input($building) . "<br><br>";

  if ($validation !== '') {
    if (($validation->hasError('requestDate'))) : echo "<div class=two>" . $validation->getError('requestDate') . "</div>";
    endif;
  }
  echo form_label('Requested Completion Date: ', 'requestDate') . "<br>" . form_input($requestDate);
  ...
  echo "<b><u>Television Request</u></b><br>";
  ...
  echo '<p>Please note that the measurements are in inches not feet.<br>At maximum, at least one side needs to be less than or equal to 42" (42" x XX" or XX" x 42").</p><br>';

  echo '<div class=one>';
  if ($validation !== '') {
    if (($validation->hasError('pAmount'))) : echo "<br><div class=two style='margin-left: 20px;'>" . $validation->getError('pAmount') . "</div>";
    endif;
  }
  ...
  if ($validation !== '') {
    if (($validation->hasError('fAmount'))) : echo "<br><div class=two style='margin-left: 20px;'>" . $validation->getError('fAmount') . "</div>";
    endif;
  }
  ...
  if ($validation !== '') {
    if (($validation->hasError('description'))) : echo "<div class=two>" . $validation->getError('description') . "</div><br>";
    endif;
  }
...
Ensure that, once the files are not zipped, the file DOES NOT exceed 7 megabytes. </p>';
  ...
  if ($validation !== '') {
    if (($validation->hasError('file'))) : echo "<div class=two>" . $validation->getError('file') . "</div>";
    endif;
  }
